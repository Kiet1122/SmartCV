...
@section('title', 'Dashboard | The Curator')

@section('content')
    <!-- Welcome Header với Animation -->
    ...
    <!-- Stats Cards với Hover Effect Đẹp Hơn -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <!-- Active Jobs -->
        <div
            class="group bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-blue-200 transition-all duration-300 cursor-pointer">
        ...
        <!-- Total Applicants -->
        <div
            class="group bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-emerald-200 transition-all duration-300 cursor-pointer">
            <div class="flex items-center justify-between mb-4">
                <div
        ...
        <!-- AI Screened -->
        <div
            class="group bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-amber-200 transition-all duration-300 cursor-pointer">
            <div class="flex items-center justify-between mb-4">
                <div
            ...
            <p class="text-xs text-gray-400 mt-1">Điểm TB: {{ number_format($avgAIScore ?? 0, 1) }}/10</p>
        </div>

        <!-- Shortlisted -->
        <div
            class="group bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-purple-200 transition-all duration-300 cursor-pointer">
            <div class="flex items-center justify-between mb-4">
                <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">Số lượng ứng viên theo ngày</h2>
                    <p class="text-gray-500 text-sm mt-1">Tuần này ({{ Carbon\Carbon::now()->startOfWeek()->format('d/m') }}
                        - {{ Carbon\Carbon::now()->endOfWeek()->format('d/m') }})</p>
                </div>
                <div class="flex gap-2">
                    <button class="px-3 py-1.5 text-xs font-medium rounded-lg bg-blue-50 text-blue-600">Tuần</button>
                    <button class="px-3 py-1.5 text-xs font-medium rounded-lg text-gray-500 hover:bg-gray-50">Tháng</button>
                </div>
            </div>
            ...
            <div class="flex justify-center gap-4 mt-6 pt-4 border-t border-gray-100">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-500">Số ứng viên mỗi ngày</span>
                </div>
            </div>
        </div>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if($avgScore)
                                    <div class="flex items-center gap-1">
                                        ...
                                        <span class="font-medium">{{ number_format($avgScore, 1) }}/10</span>
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $statusConfig = [
                                        ...
                                        'pending' => ['class' => 'bg-amber-100 text-amber-700', 'dot' => 'bg-amber-500', 'text' => 'Đang kiểm duyệt'],
                                        'closed' => ['class' => 'bg-gray-100 text-gray-700', 'dot' => 'bg-gray-500', 'text' => 'Đã đóng'],
                                        'rejected' => ['class' => 'bg-rose-100 text-rose-700', 'dot' => 'bg-rose-500', 'text' => 'Bị từ chối']
                                    ];
                ...
                @foreach($recentApplications->take(5) as $application)
                    <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900">{{ $application->candidate->user->name ?? 'Ứng viên' }}</p>
                        ...
                        <div class="flex items-center gap-4">
                            @if($application->match_score)
                                <div class="flex items-center gap-1 px-2 py-1 bg-amber-50 rounded-full">
                                    <span class="material-symbols-outlined text-amber-500 text-sm">star</span>
                                    <span
                                        class="text-sm font-semibold text-amber-600">{{ number_format($application->match_score, 1) }}/10</span>
                                </div>
                            @endif
